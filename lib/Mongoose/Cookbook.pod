...
To quickly load your classes, use the C<load_schema> method:

    package main;
    Mongoose->load_schema( search_path=>'MyApp::Schema', shorten=>1 );

Use it only once in your program. Your modules will be required
and may be used from anywhere else. 

The C<shorten> option will alias C<MyApp::Schema::MyClass> into
C<MyClass> for your convenience.
